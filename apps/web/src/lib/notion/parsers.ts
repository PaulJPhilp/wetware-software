...
import { extractCheckbox, extractDate, extractMultiSelectNames, extractNumber, extractRichText, extractSelectName, extractTitle } from './type-guards';
...
export function safeExtractProperty<T>(
    ...
    try {
        ...
        logger.parsing.error(
            `Failed to extract property '${propertyName}'`,
            page.id,
            error,
...
export function parsePostsFromResponse(response: NotionDatabase): PostMeta[] {
    ...
    if (errors.length > 0) {
        logger.warn(
            `Parsed ${posts.length} posts with ${errors.length} errors`,
            {
...
export function parseSeriesFromResponse(response: NotionDatabase): SeriesMeta[] {
    ...
    if (errors.length > 0) {
        logger.warn(
            `Parsed ${series.length} series with ${errors.length} errors`,
            {
