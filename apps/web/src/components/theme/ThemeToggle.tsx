...
export function ThemeToggle({ className = "" }: ThemeToggleProps) {
  ...
  return (
    ...
    >
      <Sun className={`h-3 w-3 transition-opacity absolute ${isMounted && !isDark ? "opacity-0" : "opacity-100"}`} />
      <Moon className={`h-3 w-3 transition-opacity ${isMounted && isDark ? "opacity-0" : "opacity-100"}`} />
