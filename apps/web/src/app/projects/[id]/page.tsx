    ...
    return (
        <main className="max-w-6xl mx-auto py-12 px-4 space-y-12">
                    ...
                    <div className={`flex-1 ${!project.imageUrl ? 'lg:w-full' : ''}`}>
                        <div className="space-y-4">
                            ...
                            <div className="flex flex-wrap gap-3 pt-4">
                                <Button asChild>
                                    <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" aria-label={`View ${project.name} source on GitHub`}>
                                ...
                                {project.demoUrl && (
                                    <Button variant="outline" asChild>
                                        <a href={project.demoUrl} target="_blank" rel="noopener noreferrer" aria-label={`Open ${project.name} live demo`}>
            ...
            {project.longDescription && (
                <section className="space-y-6">
                    <h2 className="text-2xl font-sans font-bold text-charcoal">About This Project</h2>
                    <Card>
            ...
            {relatedProjects.length > 0 && (
                <section className="space-y-6">
                    <h2 className="text-2xl font-sans font-bold text-charcoal">Related Projects</h2>
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                        {relatedProjects.map((relatedProject) => (
                            <ProjectCard key={relatedProject.id} project={relatedProject} showDescription={false} />
                        ))}
                    </div>
                </section>
            )}
            ...
            <section className="bg-silver/30 dark:bg-charcoal/30 rounded-xl p-8 text-center space-y-6">
                <div className="space-y-4">
                    <h2 className="text-2xl font-sans font-bold text-charcoal">Interested in This Project?</h2>
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                    <Button asChild>
                        <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" aria-label={`View ${project.name} on GitHub`}>
                            <Github className="w-4 h-4 mr-2" />
                            View on GitHub
                        </a>
                    </Button>
