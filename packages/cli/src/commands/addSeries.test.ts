...
describe("addSeries", () => {
  it("should add a series", async () => {
    const mockNotion = createNotionMock({
      addSeries: vi.fn(() =>
        Effect.succeed({ pageId: "123", url: "http://example.com" })
      ),
    });
    const mockAI: OpenAIService = {
      generateSeriesJson: vi.fn(() =>
        Effect.succeed(
          '{ "name": "test", "description": "test", "goal": "test" }'
        )
      ),
      generateResourceJson: vi.fn((_args) =>
        notImplementedEffect<string>("generateResourceJson")
      ),
