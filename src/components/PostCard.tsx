...
export function PostCard({ post }: { post: Post }) {
	...
	return (
		<Card className="group relative flex flex-col px-1 py-1 gap-0 text-[8px] border-silver hover:border-orange transition-colors duration-200 hover:shadow-lg min-h-0 w-[90%]">
			<CardHeader className="space-y-0 pb-0 px-1 py-0">
				<div className="flex items-center space-x-1">
					<Badge variant="secondary" className="font-sans text-[8px] px-1 py-[1px] bg-silver text-charcoal">
						{post.type}
					</Badge>
					<FocusIcon className="w-1.5 h-1.5 text-orange" />
					<time className="text-[8px] text-charcoal/60">{post.publishDate}</time>
				</div>
				{post.seriesName && post.partNumber && (
					<div className="text-[8px] text-orange font-medium">
						{post.seriesName} • Part {post.partNumber}
					</div>
				)}
			</CardHeader>
			<CardContent className="flex-1 space-y-[2px] flex flex-col justify-center px-1 py-0">
				<h2 className="text-[10px] font-sans font-bold group-hover:text-orange transition-colors line-clamp-2">
					{post.name}
				</h2>
				<p className="text-[7px] text-charcoal/80 line-clamp-3 leading-snug">{post.description}</p>
			</CardContent>
			<CardFooter className="flex items-center space-x-1 text-[8px] text-charcoal/60 px-1 py-0">
				<div className="flex items-center space-x-1">
					<div className="space-x-1">
						{post.tags.map((tag) => {
							const colorClass = notionColorToTailwind[tag.color] || notionColorToTailwind.default;
							return (
								<Badge key={tag.name} variant="outline" className={`font-sans text-[7px] px-0.5 py-[1px] ${colorClass}`}>
									{tag.name}
								</Badge>
							);
						})}
